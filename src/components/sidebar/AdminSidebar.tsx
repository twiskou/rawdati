...
const navItems = [
  ...
  { href: '/admin/children', icon: Baby, label: 'Enfants' },
  { href: '/admin/classrooms', icon: DoorOpen, label: 'Classes' },
  { href: '/admin/activities', icon: Calendar, label: 'Activités' },
...
export default function AdminSidebar({ firstName, lastName, avatar, kindergartenName }: AdminSidebarProps) {
  ...
  const SidebarContent = () => (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', padding: '24px 16px' }}>
      <div style={{ marginBottom: '32px', paddingLeft: '8px' }}>
        ...
        {kindergartenName && (
          <p style={{ margin: 0, fontSize: '11px', color: '#94a3b8', paddingLeft: '48px' }}>{kindergartenName}</p>
        )}
      </div>
      ...
      <div style={{ background: 'linear-gradient(135deg, rgba(247,37,133,0.06), rgba(67,97,238,0.06))', borderRadius: '14px', padding: '14px', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <div style={{ overflow: 'hidden' }}>
          ...
          <p style={{ margin: 0, fontSize: '11px', color: '#64748b' }}>Administrateur</p>
        </div>
      </div>
      ...
      <nav style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '4px' }}>
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', borderRadius: '12px', textDecoration: 'none', background: isActive ? 'linear-gradient(135deg, rgba(247,37,133,0.10), rgba(67,97,238,0.10))' : 'transparent', color: isActive ? '#4361EE' : '#64748b', fontWeight: isActive ? '600' : '500', fontSize: '14px', transition: 'all 0.15s ease', borderLeft: isActive ? '3px solid #4361EE' : '3px solid transparent' }}>
              <Icon size={18} />
              {item.label}
            </Link>
          )
  ...
  return (
    <>
      <aside style={{ width: '260px', minHeight: '100vh', background: 'white', borderRight: '1px solid #f1f5f9', position: 'fixed', top: 0, left: 0, bottom: 0, zIndex: 40, overflowY: 'auto' }} className="hidden-mobile-sidebar">
        <SidebarContent />
      </aside>
      ...
      <div style={{ position: 'fixed', top: 0, left: 0, bottom: 0, width: '280px', background: 'white', zIndex: 70, transform: mobileOpen ? 'translateX(0)' : 'translateX(-100%)', transition: 'transform 0.3s ease', overflowY: 'auto' }}>
